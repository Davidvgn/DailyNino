 ...
 * In Kotlin, every lambda created is an anonymous class, so it can't be equal to any other. This is a problem when trying to match the
 * equality of 2 models (the expected one and the tested one) in unit testing.
 *
 * So every EquatableCallback are equals to each other, allowing the use of a simple "assertEquals()" in unit test.
 *
